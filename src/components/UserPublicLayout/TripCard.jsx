...
const TripCard = ({ trip }) => {
  ...
  return (
    <div className="w-full bg-white rounded-3xl overflow-hidden shadow-md border border-slate-200 flex flex-col md:flex-row hover:shadow-lg transition-all duration-300 group">
      ...
      <div className="flex-1 p-6 flex flex-col justify-between gap-4">
        ...
        <div className="flex items-center justify-between gap-4 py-2 border-y border-slate-200">
          ...
          <div className="flex-grow flex flex-col items-center relative">
            ...
            <span className={`text-[11px] font-bold ${trip.isDirect ? 'text-slate-500' : 'text-emerald-600 font-extrabold'}`}>
              {trip.stopoverType}
            </span>
          </div>
      ...
      <div className="hidden md:flex w-44 border-l border-slate-200 p-6 flex-col justify-between items-center text-center shrink-0 bg-slate-50">
        ...
        <div className="flex flex-col gap-2 w-full mt-4">
          ...
          <button className="w-full bg-slate-200/70 hover:bg-slate-200 text-slate-700 text-xs font-extrabold py-3 px-4 rounded-xl cursor-pointer transition-colors duration-300">
            Chi tiết
          </button>
        </div>
      </div>
