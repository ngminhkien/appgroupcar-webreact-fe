...
const EditBusRouteModal = ({ isOpen, onClose, onUpdated, route }) => {
      ...
      const fetchData = async () => {
        ...
        try {
          const [detailRes, locRes] = await Promise.all([
            getBusRouteByIdApi(route.id),
            getLocationsApi({ PageSize: 1000 })
          ]);
          
          if (cancelled) return;
          
          const detailData = detailRes?.data?.items ? detailRes.data.items[0] : (detailRes?.data ?? detailRes);
          const locData = locRes?.data?.items || locRes?.items || locRes?.data || locRes || [];
          ...
          if (!cancelled) {
            toast.error('Lỗi khi tải dữ liệu chi tiết tuyến xe');
            onClose();
          }
        } finally {
          if (!cancelled) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.success(res?.message || 'Cập nhật tuyến xe thành công!');
      if (onUpdated) onUpdated();
      onClose();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Có lỗi xảy ra khi cập nhật tuyến xe.');
    } finally {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in-95 duration-200">
        <div className="px-6 pt-6 pb-4 border-b border-slate-100 flex items-center justify-between shrink-0">
          <h2 className="text-xl font-semibold text-slate-800">Cập nhật tuyến xe</h2>
...
export default EditBusRouteModal;
