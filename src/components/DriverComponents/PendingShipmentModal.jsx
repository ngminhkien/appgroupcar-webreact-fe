...
const PendingShipmentModal = ({ offerId, onClose }) => {
  ...
  // Handle Reject Shipment proposal
  const handleReject = async (shipmentOfferId) => {
    if (!window.confirm('Bạn có chắc chắn muốn từ chối yêu cầu vận chuyển này?')) {
      return;
    }
    setProcessingId(shipmentOfferId);
    try {
        ...
        <div className="p-6 border-b border-slate-100 flex items-center justify-between text-left">
          <div>
            <h2 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2">
              ...
              Duyệt yêu cầu vận chuyển (Xe tải)
            </h2>
