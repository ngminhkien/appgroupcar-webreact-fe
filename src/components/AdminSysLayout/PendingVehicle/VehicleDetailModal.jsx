...
import { approveVehicleApi, getVehicleByIdApi } from '@/services/vehicleService';
import { VehicleStatus, VehicleType } from '@/types/enums';

const VehicleDetailModal = ({ isOpen, onClose, vehicle, onUpdated }) => {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setIsLoading(true);
      try {
        const response = await getVehicleByIdApi(vehicle.id);
  ...
  const handleApprove = async () => {
    ...
    try {
      const res = await approveVehicleApi(vehicle.id);
      toast.success(res?.message || 'Xét duyệt phương tiện thành công!');
      if (onUpdated) onUpdated();
      onClose();
    } catch (error) {
      toast.error(error.response?.data?.message || 'Có lỗi xảy ra khi xét duyệt.');
    } finally {
    ...
    >
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 border-b border-slate-100 shrink-0">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl font-semibold text-slate-800">Chi tiết phương tiện</h2>
            </div>
            <button
              type="button"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-1.5 md:col-span-2">
                  <label className="block text-sm font-medium text-slate-500">Chủ xe/Tài xế</label>
        ...
        <div className="px-6 py-4 border-t border-slate-100 bg-slate-50/80 flex justify-end gap-3 shrink-0">
          ...
          {detailData?.status === VehicleStatus.Pending && (
             <>
               <button
                 type="button"
